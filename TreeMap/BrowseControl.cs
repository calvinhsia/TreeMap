...
public class BrowseControl : DockPanel
{
    ...
    public BrowseControl(IEnumerable query, int[]? colWidths = null, bool filterOnLeft = true, int rowHeight = DefaultRowHeight)
    {
        ...
        catch (Exception ex)
        {
            ...
            Trace.WriteLine("BrowseControl creation failed: " + ex);
            throw;
        }
    }
...
#pragma warning restore CS0414
    ...
    public object? SelectedItem => _listBox?.SelectedItem;
        ...
        // Raised when selection changes in the underlying ListBox
    ...
    public ListBoxBrowseView(IEnumerable query, BrowseControl browseControl)
    {
        ...
        if (ienum == null)
        {
            // fallback: infer from first item runtime type
            itemType = itemsList.Count > 0 ? itemsList[0].GetType() : typeof(object);
        }
        else
        {
            itemType = ienum.GetGenericArguments()[0];
            if (itemType == typeof(object) && itemsList.Count > 0)
            {
                // if IEnumerable<object>, try inferring runtime type
                itemType = itemsList[0].GetType();
            }
        }
    ...
    private void BuildVisualStructure()
    {
        ...
        {
            var sel = _listBox.SelectedItems?.Cast<object>().ToList() ?? new List<object>();
    ...
    private async void OnCopyClick(object? sender, RoutedEventArgs e)
    {
        ...
        {
            Trace.WriteLine("Copy to clipboard failed: " + ex);
        }
    }
    ...
    private async void OnExportCsvClick(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                Trace.WriteLine("Could not open CSV file: " + openEx);
            }
        }
        catch (Exception ex)
        {
            Trace.WriteLine("Export to CSV failed: " + ex);
        }
    }
    ...
    private async void OnExportTxtClick(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                Trace.WriteLine("Could not open TXT file: " + openEx);
            }
        }
        catch (Exception ex)
        {
            Trace.WriteLine("Export to Notepad failed: " + ex);
        }
    }
    ...
    private void OnHeaderClick(int columnIndex)
    {
        ...
        {
            Trace.WriteLine($"Error sorting by {col.HeaderText}: {ex.Message}");
        }
    }
    ...
    public void AddContextMenuItem(string itemName, string tooltip, Action<IList<object>> action)
    {
        ...
        menuItem.Click += (s, e) =>
        {
            ...
            {
                Trace.WriteLine($"Context menu action '{itemName}' failed: " + ex);
            }
        };
